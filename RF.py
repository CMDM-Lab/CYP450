...
# Optimizing function for RF
def RF_loss_function(hyperparams):
    # Fill in the parameters to be optimized, the following is just an example
    ne, crit, md, mss, msl, mwf, mf, cw = hyperparams['ne'], hyperparams['crit'], hyperparams['md'], hyperparams['mss'], \
                                          hyperparams['msl'], hyperparams['mwf'], hyperparams['mf'], hyperparams['cw']
...
hyperparams = {
    'ne': scope.int(hp.quniform('ne', 50, 150, q=1)),
    'crit': hp.choice('crit', criterion),
    'md': scope.int(hp.quniform('md', 10, 30, q=1)),
...
clf = RandomForestClassifier(n_estimators=int(best['ne']),
                             criterion=criterion[best['crit']], # If the range is defined by a list, the value recorded in best will be the index of the list
...
Result['n_estimators'].append(int(best['ne']))
