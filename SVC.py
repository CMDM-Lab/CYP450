...
# The following code uses hyperopt to optimize the model's hyperparameters; if not needed, it can be skipped
# Optimizing function for SVM
def SVM_loss_function(hyperparams):
    # Fill in the parameters to be optimized, the following is just an example
    c, kn, gm, c0, tol, cs, dfs, cw = hyperparams['C'], hyperparams['kernel'], hyperparams['gamma'], hyperparams[
...
hyperparams = {
                ...
                'decision_function_shape': hp.choice('decision_function_shape', decision_function_shape),
...
clf = make_pipeline(StandardScaler(), SVC(C=int(best['C']),
                                      kernel= kernel[best['kernel']], # If the range is defined by a list, the value recorded in best will be the index of the list
                                      gamma= gamma[best['gamma']],
                                      coef0= int(best['coef0']),
                                      tol= tol[best['tol']],
                                      cache_size =  int(best['cache_size']),
                                      decision_function_shape=decision_function_shape[best['decision_function_shape']],
...
Result['decision_function_shape'].append(decision_function_shape[best['decision_function_shape']])
Result['class_weight'].append(class_weight[best['class_weight']])
